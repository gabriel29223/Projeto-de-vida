...
// 1. Estudar para o Enem 2026 - 1º de novembro de 2026 (primeiro domingo de novembro)
// 2. Terminar a escola - 31 de dezembro de 2026 (final do ano letivo)
// 3. Ingressar na faculdade - 1 de abril de 2027
// 4. Comprar primeiro carro - 1 de junho de 2027
...
function getDataFaculdade() {
    return new Date(2027, 3, 1, 0, 0, 0); // 1 de abril de 2027
}

function getDataCarro() {
    return new Date(2027, 5, 1, 0, 0, 0); // 1 de junho de 2027
}

const datas = [getDataEnem, getDataFinalEscola, getDataFaculdade, getDataCarro];
    ...
    if (diasElement && horasElement && minutosElement && segundosElement) {
        ...
        if (tempo.concluido) {
            ...
            if (abaConteudo && !abaConteudo.querySelector('.mensagem-concluido')) {
                ...
                mensagem.style.cssText = 'background: linear-gradient(135deg, #11998e, #38ef7d); color: white; padding: 10px; border-radius: 10px; margin-top: 20px; font-weight: bold; text-align: center;';
